...
On a computer, all text is an encoded sequences of characters. (A *character* is a symbol like a letter or number, a space/tab/newline, or special characters like ? or emojis.)

Unicode is the global standard for text encoding. It assigns every character in every language to a code point. For example, capital `A` is `U+0041`, lowercase `a` is `U+0061`, and the number `1` is `U+0031`. Some other examples are below.
...
The job of *vector space embedding* is to assign coordinates to every word in the vocabulary, and do this so that similar words are relatively together and dissimilar words are relatively far apart.
